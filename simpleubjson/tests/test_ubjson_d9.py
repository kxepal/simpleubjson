...
class EncoderTestCase(Draft9TestCase):
    ...
    def test_custom_default_handler(self):
        ...
        self.assertEqual(data, b('ASB\x08sentinelE'))
...
class HugeNumberTestCase(Draft9TestCase):
    ...
    def test_decode(self):
        source = b('HB\x33314159265358979323846264338327950288419716939937510')
    ...
    def test_encode(self):
        ...
        expected = b('HB\x33314159265358979323846264338327950288419716939937510')
        data = self.encode(source)
        self.assertEqual(data, expected)

    def test_encode_decimal(self):
        source = Decimal('3.14')
        expected = b('HB\x043.14')
        data = self.encode(source)
        self.assertEqual(data, expected)

    def test_fail_on_invalid_numeric_value(self):
        source = b('HB\x33314159 65358979323846264338327950288419716939937510')
        self.assertRaises(ArithmeticError, self.decode, source)

    def test_fail_on_non_numeric_value(self):
        source = b('HB\x09foobarbaz')
        self.assertRaises(ArithmeticError, self.decode, source)
...
class StringTestCase(Draft9TestCase):
    ...
    def test_decode_ascii(self):
        data = self.decode(b('SB\x03foo'))
        self.assertEqual(data, 'foo')

    def test_encode_ascii(self):
        data = self.encode('foo')
        self.assertEqual(data, b('SB\x03foo'))

    def test_decode_utf8(self):
        source = b('SB\x0c\xd0\xbf\xd1\x80\xd0\xb8\xd0\xb2\xd0\xb5\xd1\x82')
        data = self.decode(source)
        self.assertEqual(data, u('привет'))

    def test_encode_utf8(self):
        expected = b('SB\x0c\xd0\xbf\xd1\x80\xd0\xb8\xd0\xb2\xd0\xb5\xd1\x82')
...
class ArrayTestCase(Draft9TestCase):
    ...
    def test_decode_array(self):
        data = list(self.decode(b('AB\x01B\x02B\x03E')))
        self.assertEqual(data, [1, 2, 3])

    def test_encode_array(self):
        data = self.encode([1, 2, 3])
        self.assertEqual(data, b('AB\x01B\x02B\x03E'))
...
class StreamTestCase(Draft9TestCase):
    ...
    def test_decode_unsized_array(self):
        data = self.decode(b('AB\x01B\x02B\x03E'))
    ...
    def test_encode_generator(self):
        ...
        self.assertEqual(data, b('AB\x00B\x01B\x02B\x03B\x04B\x05B\x06E'))

    def test_decode_unsized_object(self):
        data = self.decode(b('OSB\x03fooSB\x03barSB\x03barSB\x03bazE'))
        self.assertTrue(isinstance(data, GeneratorType))
        self.assertEqual(dict(data), {'foo': 'bar', 'bar': 'baz'})

    def test_decode_unsized_array_with_noops(self):
        data = self.decode(b('ANB\x01NNNB\x02NNNNNNNNNNNNNB\x03E'))
        self.assertTrue(isinstance(data, GeneratorType))
        self.assertEqual(list(data), [1, 2, 3])

    def test_decode_nested_unsized_values(self):
        data = self.decode(b('AAB\x2aEOSB\x03fooB\x2aEE'))
        self.assertTrue(isinstance(data, GeneratorType))
        item = data.next()
    ...
    def test_encode_dict_itervalues(self):
        ...
        self.assertEqual(data, b('AB\x02B\x00B\x01E'))
    ...
    def test_encode_dict_iteritems(self):
        ...
        self.assertEqual(
            data,
            b('OSB\x03bazB\x02SB\x03fooB\x00SB\x03barB\x01E')
        )
...
class ObjectTestCase(Draft9TestCase):
    ...
    def test_decode_object(self):
        data = dict(self.decode(b('OSB\x03fooSB\x03barSB\x03barSB\x03bazE')))
        self.assertEqual(data, {'foo': 'bar', 'bar': 'baz'})

    def test_encode_object(self):
        data = self.encode({'foo': 'bar', 'bar': 'baz'})
        self.assertEqual(data, b('OSB\x03fooSB\x03barSB\x03barSB\x03bazE'))

    def test_decode_object_with_nested_unsized_objects(self):
        source = b('OSB\x03barAB\x2aESB\x03bazONNNSB\x03fooB\x2aEE')
    ...
    def test_fail_decode_if_eos_marker_occurres_in_sized_object(self):
        data = b('o\x02SB\x03fooSB\x03barSB\x03barESB\x03baz')
        self.assertRaises(ValueError, self.decode, data)

    def test_should_skip_noop_markers_in_sized_object(self):
        data = dict(self.decode(b('OSB\x03fooSB\x03barNNNSB\x03barSB\x03bazE')))
        self.assertEqual(data, {'foo': 'bar', 'bar': 'baz'})

    def test_fail_decode_non_string_object_keys(self):
        self.assertRaises(ValueError,
                          list,
                          self.decode(b('OB\x03SB\x03barE')))

    def test_fail_decode_on_early_end(self):
        self.assertRaises(ValueError, list, self.decode(b('OB\x01')))
